...
{      
	...
	// of even length starting from the root node that ends on the
	// vertex. To find this path, the backtrace() function is called,
...
{            
	...
	// the augmentMatching() procedure. Each element contains the vertex
...
// is in. This, together with labels eliminates the need to
// contract the graph.
int blossom[MAX];        

// The path arrays are where the backtrace() routine will
// store the paths it finds. Only two paths need to be
// stored. endPath[p] denotes the end of path[p].
int path[2][MAX], endPath[2];  

// An array of flags. match[i] stores if vertex i is in the matching.
bool match[MAX];  

// label[i] contains the label assigned to vertex i. It may be undefined,
// empty (meaning the node is a root) and a node might have even and odd
// labels at the same time, which is the case for nonbase nodes of blossoms
Label label[MAX]; 

// The queue is necessary for efficiently scanning all labels.
elem queue[2 * MAX];         

// A label is enqueued when assigned and dequeued after scanned.
...
{
	int n, e, a, b;
	scanf_s(" %d %d", &n, &e);      //The graph is read and its edges are unmatched by default.
...
// function begin. At the start, all labels are undefined,
...
// Enqueues labels for scanning.
// No label that's dequeued during the execution
// of augmentMatching() goes back to the queue.
// Thus, circular arrays are unnecessary.
...
// goal of finding augmenting paths or
// concluding that the matching is maximum.
bool augmentMatching(int n)
{  
	int node, nodeLabel;       
	initAlg(n);   

	// Initialize the queue with the exposed vertices,
