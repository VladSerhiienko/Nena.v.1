...
[maxvertexcount(3)] void main(in point GSInput input[1], inout TriangleStream<PSInput> output)
{
	...
	for (uint i = 0; i < 3; i++)
	{
		...
		//printf("pos o %f %f %f", element.Position.x, element.Position.y, element.Position.z);
		element.Position = mul(element.Position, _Vm);
		//printf("    v %f %f %f", element.Position.x, element.Position.y, element.Position.z);
		element.Position = mul(element.Position, _Pm);
		//printf("    p %f %f %f", element.Position.x, element.Position.y, element.Position.z);
