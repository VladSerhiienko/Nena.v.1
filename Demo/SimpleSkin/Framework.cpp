	...
	)
{
	...
	::SetWindowText(handle, __TEXT("Simple Skin"));
	...
	if (SUCCEEDED(error))
	{
		...
		window->Content.Posed->Root->UpdateTransformWS();
		window->Content.Pose();
		...
		//window->Renderer.Pbdata = &window->Content.Mesh->PointListOS;
		//window->Renderer.Nbdata = &window->Content.Mesh->NormalListOS;
