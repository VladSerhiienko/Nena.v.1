...
void Sandbox::FluidSimulator::ComputeExternalForces( )
{
	...
	SplatYImpulse( ProblemHeight( ) * 0.5f + s_positionY ); // + s_positionY * _To_single rand( ) / _To_single RAND_MAX );
	SplatXImpulse( ProblemHeight( ) * 0.5f + s_positionX ); // + s_positionX * _To_single rand( ) / _To_single RAND_MAX );
	...
	if ( s_iteration < 200u )
	{
		...
		ForceXMagnitude( randx * 500.0f );
		ForceYMagnitude( randy * 500.0f );
#endif
