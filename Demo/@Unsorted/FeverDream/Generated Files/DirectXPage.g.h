

#pragma once
//------------------------------------------------------------------------------
//     This code was generated by a tool.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
//------------------------------------------------------------------------------

namespace Windows {
    namespace UI {
        namespace Xaml {
            namespace Controls {
                ref class SwapChainBackgroundPanel;
                ref class TextBlock;
                ref class TextBox;
                ref class Button;
            }
        }
    }
}

namespace FeverDream
{
    partial ref class DirectXPage : public ::Windows::UI::Xaml::Controls::Page, 
        public ::Windows::UI::Xaml::Markup::IComponentConnector
    {
    public:
        void InitializeComponent();
        virtual void Connect(int connectionId, ::Platform::Object^ target);
    
    private:
        bool _contentLoaded;
    
        private: ::Windows::UI::Xaml::Controls::SwapChainBackgroundPanel^ SwapChainPanel;
        private: ::Windows::UI::Xaml::Controls::TextBlock^ SimpleTextBlock;
        private: ::Windows::UI::Xaml::Controls::TextBox^ HostTextBox;
        private: ::Windows::UI::Xaml::Controls::TextBox^ ServiceTextBox;
        private: ::Windows::UI::Xaml::Controls::Button^ OpenConnectionButton;
        private: ::Windows::UI::Xaml::Controls::Button^ CloseConnectionButton;
    };
}

