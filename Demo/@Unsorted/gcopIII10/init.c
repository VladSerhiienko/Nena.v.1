...
*/
void
assign_probab(VECTOR fProbVector, int iPopSize, float fQ)
{
    int             i;
    float          Q1;
...
** int		iDirection - UP if the last operator improved things, DOWN if not.
**
*/
void reassign_probab(VECTOR fProbVector, int iOpNum, int iNumOperators,
						int iDirection)
{
	...
	if (iDirection == UP)
	{
		if (fProbVector[iOpNum] + INCREMENT <= U_FREQ_LIM)
			fProbVector[iOpNum] = fProbVector[iOpNum] + INCREMENT;
	}
	
	if (iDirection == DOWN)
	{
		if (fProbVector[iOpNum] - INCREMENT >= L_FREQ_LIM)
			fProbVector[iOpNum] = fProbVector[iOpNum] - INCREMENT;
	}
...
*/
void
init_ref_pop(EVOLDATA ev, MATRIX fRefPop, MATRIX fDC, MATRIX fLC)
{
	...
	if (ev.iRefInitType == SINGLE)
	{
		...
		while ((iFlag == FALSE) && (iTries <= NTRIES))
		{
			...
			for (j=1; j<=ev.iNumVars; j++)
			{
				...
				/* if no DC, generate values between limits */
				if (iDCFlag == FALSE)
					fRefPop[1][j] = randgen(MINNUM, MAXNUM);
	...
	{	
		...
		while (iPcount<=ev.iRefPopSize)
		{
			...
			while ((iFlag == FALSE) && (iTries <= NTRIES))
			{
				...
				/* if no DC, generate values between limits */
				if (iDCFlag == FALSE)
					fRefPop[iPcount][j] = randgen(MINNUM, MAXNUM);
