...
void
optimize(EVOLDATA ev, MATRIX fLC, MATRIX fDC, MATRIX fSearchPop, VECTOR fSearchProbDist,
MATRIX fRefPop, VECTOR fRefProbDist, VECTOR fOpProbDist, FILE
*outfile,
VECTOR fNormProbDist)
{
	...
	fprintf(outfile,
		"-- Initial Evaluation -----------------------------------\n");
	fprintf(outfile, "Evaluation Count: %10ld;Best Ref. Val = %10.12f\n\n",
		lEvalCount, fBestRefVal);

	printf("Evaluation Count: %10ld;Best Ref. Val = %10.12f\n\n",
		lEvalCount, fBestRefVal);
	...
	while (lEvalCount <= ev.lTotEvals)
	{
		...
		if (check_LC(ev, fOffspring, fLC, fDC) == TRUE)
		{
		...
		if (ev.iRefPeriod != 0)
		{
			if ((lEvalCount % ev.iRefPeriod) == 0)
			{
				for (iCount = 1; iCount <= ev.iRefOffspring; iCount++)
				{
					...
					if (check_all(ev, fOffspring, fLC, fDC) == TRUE)
					{
		...
		case MAX:
			if (fRefPop[1][0] > fBestRefVal)
			{
				...
				for (i = 1; i <= ev.iNumVars; i++)
					fBestVec[i] = fRefPop[1][i];
				fprintf(outfile,
					"Evaluation Count: %10ld ;Best Ref. Val = %10.12f\n",
					lEvalCount, fBestRefVal);
				printf("Evaluation Count: %10ld;Best Ref. Val = %10.12f\n",
					lEvalCount, fBestRefVal);
			}
