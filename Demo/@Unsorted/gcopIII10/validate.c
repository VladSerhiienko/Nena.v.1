...
int check_LC(EVOLDATA ev, VECTOR X, MATRIX fLC, MATRIX fDC)
{
	...
	/* for ANY variable, if DC is not met, iFlag1 is FALSE */
...
int check_all(EVOLDATA ev, VECTOR X, MATRIX fLC, MATRIX fDC)
{
	...
	float nlineq[MAXVECTOR], nleq[MAXVECTOR], fQ;
...
#include "nleq.inl"
	...
	iFlag1 = TRUE; iFlag2=TRUE; iFlag3=TRUE;
	...
	if ((iFlag1 == TRUE) && (iFlag2 == TRUE) && (iFlag3 == TRUE))
		iFlag = TRUE;
	else
		iFlag = FALSE;
