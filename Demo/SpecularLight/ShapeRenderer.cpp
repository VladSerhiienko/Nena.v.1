...
void Demo::ShapeRenderer::UpdateBuffers()
{
	...
	Lbdata.S.x = (Light->SpecularColor.x);
	Lbdata.S.y = (Light->SpecularColor.y);
	Lbdata.S.z = (Light->SpecularColor.z);
	Lbdata.S.w = (Light->SpecularPower);

	/*
	Cbdata.P.x = (Camera->EyeDisplacement.x);
	Cbdata.P.y = (Camera->EyeDisplacement.y);
	Cbdata.P.z = (Camera->EyeDisplacement.z);
	*/
	///*
	Cbdata.P.x = (Camera->EyePosition.x);
	Cbdata.P.y = (Camera->EyePosition.y);
	Cbdata.P.z = (Camera->EyePosition.z);
	//*/

	Lbdata.D = ::Nena::XMVector4Transform(Lbdata.D, Xbdata.V);

	Xb.SetData(__D3Dc(), Xbdata);
	Cb.SetData(__D3Dc(), Cbdata);
...
void Demo::ShapeRenderer::Render()
{
	...
	__D3Dc()->GSSetConstantBuffers(1, 1, Cb.GetBufferAddress());
