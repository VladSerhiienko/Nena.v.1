...
int DemoObjects1()
{
	...
	while (!GetAsyncKeyState(VK_ESCAPE))
	{
		...
		INT32 targets = card.Tracker->GetTargetCount();
		for (INT32 target = 0; target < targets; target++)
		{
			if (card.Tracker->GetTargetStatus(target) == PXCDCVTracker::STATUS_TRACKING)
			{
				wprintf(L"[Loop] Tracking [%s]\n", card.Tracker->GetTargetName(target));
			}
		}
