...
namespace DirectX
{
	...
	Resources::IWICImagingFactory* _GetWIC()
	{
		static Resources::IWICImagingFactory* s_Factory = nullptr;
		...
		else
		{
			hr = CoCreateInstance(
				...
				nullptr,
				CLSCTX_INPROC_SERVER,
				__uuidof(Resources::IWICImagingFactory),
				(LPVOID*) &s_Factory
				);
...
{
	Resources::IWICImagingFactory* pWIC = _GetWIC();
	if (!pWIC)
		return 0;
...
static HRESULT CreateTextureFromWIC(_In_ ID3D11Device* d3dDevice,
	...
	_In_ Resources::IWICBitmapFrameDecode *frame,
	_In_ size_t maxsize,
	...
	_Out_opt_ ID3D11ShaderResourceView** textureView)
{
	...
	else if (twidth != width || theight != height)
	{
		...
		{
			ScopedObject<Resources::IWICFormatConverter> FC;
	...
	if (SUCCEEDED(hr) && tex != 0)
	{
		...
		{
			Assist::SetDebugName(tex, "NenaWICTextureLoader");
			//SetDebugObjectName(tex, "NenaWICTextureLoader");
			tex->Release();
		}
	}
...
HRESULT Nena::Utility::CreateWICTextureFromMemory(
...
_Use_decl_annotations_
HRESULT Nena::Utility::CreateWICTextureFromMemoryEx(
	IDirect3DDevice* d3dDevice,
	IDirect3DContext* d3dContext,
	const uint8_t* wicData,
	size_t wicDataSize,
	...
	)
{
	...
	ScopedObject<Resources::IWICStream> stream;
	...
	// Initialize WIC
	ScopedObject<Resources::IWICBitmapDecoder> decoder;
	hr = pWIC->CreateDecoderFromStream(stream.Get(), 0, WICDecodeMetadataCacheOnDemand, &decoder);
	if (FAILED(hr))
		return hr;

	ScopedObject<Resources::IWICBitmapFrameDecode> frame;
	...
	{
		Assist::SetDebugName(*texture, "NenaWICTextureLoader");
		//SetDebugObjectName(*texture, "NenaWICTextureLoader");
	}

	if (textureView != 0 && *textureView != 0)
	{
		Assist::SetDebugName(*textureView, "NenaWICTextureLoader");
		//SetDebugObjectName(*textureView, "NenaWICTextureLoader");
	}
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_
HRESULT Nena::Utility::CreateWICTextureFromFile(
	IDirect3DDevice* d3dDevice,
	IDirect3DContext* d3dContext,
...
_Use_decl_annotations_
HRESULT Nena::Utility::CreateWICTextureFromFileEx(
	IDirect3DDevice* d3dDevice,
	IDirect3DContext* d3dContext,
	const wchar_t* fileName,
	size_t maxsize,
