#include "VideoTrackingFaceLxnd.h"
	...
	namespace Video
	{
		namespace Luxand
		{
			namespace Tracking
			{
				...
				Face::Tracker::PerFrameData::PerFrameData(Tracker *host, UINT32 size, UINT32 namesLen) 
					: Host(host), Count(size), Bytes(size * sizeof INT64), NamesBytes(namesLen * sizeof CHAR)
				{
#if DEBUG //DEBUGGING
				...
				Face::Tracker::PerFrameData::~PerFrameData()
				{
#if DEBUG //DEBUGGING
					Platform::DebugTrace("FaceLuxand::Tracker::PerFrameData::~PerFrameData()\n");
#endif //NDEBUG
				...
				Face::Status Face::Tracker::PerFrameData::StoreFacePosById(_In_ INT64 id)
				{
					return Check() = FSDK_GetTrackerFacePosition(Host->Handle, Host->Device->Handle, id, &Pos);
				}

				Face::Status Face::Tracker::PerFrameData::StoreFacePosByIdIndex(_In_ INT64 idx)
				{
					return Check() = FSDK_GetTrackerFacePosition(Host->Handle, Host->Device->Handle, Ids[idx], &Pos);
				}

				Face::Status Face::Tracker::PerFrameData::StoreFaceNamesById(_In_ INT64 id)
				{
					return Check() = FSDK_GetTrackerFacePosition(Host->Handle, Host->Device->Handle, id, &Pos);
				}

				Face::Status Face::Tracker::PerFrameData::StoreFaceNamesByIdIndex(_In_ INT64 idx)
				{
				...
				Face::Status Face::Tracker::PerFrameData::SetFaceNameById(_In_ INT64 id)
				{
					return Check() = FSDK_GetTrackerFacePosition(Host->Handle, Host->Device->Handle, id, &Pos);
				}

				Face::Status Face::Tracker::PerFrameData::SetFaceNameByIdIndex(_In_ INT64 idx)
				{
				...
				Face::Tracker::Tracker(Face *faceLuxand, HTracker handle) :
				...
				Face::Tracker::PerFrameData *Face::Tracker::QueryPerFrameData(
				...
				Face::Status Face::Tracker::ClearPerFrameData()
				{
				...
				Face::Status Face::Tracker::SetParameter(
				...
				Face::Status Face::Tracker::SetMultipleParameters(
					Face::Tracker::Target target, INT32 *error
					)
				{
					String8 params;
				...
				Face::Status Face::Tracker::GrabFrame(void)
				{
					return Check() = FSDK_GrabFrame(Device->Handle, &ImageHandle);
				}

				Face::Status Face::Tracker::FeedFrame(_In_ PerFrameData *perFrame)
				{
				...
				Face::Status Face::Tracker::FreeFrame(_In_ PerFrameData *perFrame)
				{
	...
	namespace Video
	{
		namespace Luxand
		{
			namespace Tracking
			{
				...
				Face::Camera::Camera(Face *faceLuxand, INT32 index, WCHAR *name) :
				...
				Face::Status Face::Camera::SetFormat(INT32 formatIndex)
				{
				...
				Face::Status Face::Camera::SetFormat(VideoFormat &format)
				{
				...
				Face::Status Face::Camera::QueryFormats()
				{
#if DEBUG //DEBUGGING
...
#if DEBUG //DEBUGGING
							for (UINT32 fmt = 0; fmt < FormatCount; fmt++)
							{
								Platform::DebugTrace(
				...
				Face::Status Face::Camera::ClearFormats()
				{
					if (!FormatCount) return FSDKE_OK; else
					{
						Format = NULL;
...
namespace Nena
{
	namespace Video
	{
		namespace Luxand
		{
			namespace Tracking
			{
				UINT32 Face::sm_instanceCount = 0u;
				BOOL Face::sm_activated = FALSE;
				BOOL Face::sm_initialized = FALSE;
				BOOL Face::sm_capturingInitialized = FALSE;

				Face::Face() : Utility::BasicStatus<INT32>(),
				...
				Face::Status Face::DeleteTracker(Tracker *tracker)
				{
				...
				Face::Status Face::ClearTrackers()
				{
#if DEBUG //DEBUGGING
				...
				Face::Status Face::SetDevice(UINT32 deviceIndex)
				{
					if (!Devices.empty())
					{
				...
				Face::Status Face::SetDeviceByName(String16 deviceName)
				{
					if (Devices.size())
					{
						if (deviceName.empty())
				...
				Face::Status Face::SetDeviceByName(String8 deviceName)
				{
					return SetDeviceByName(Nena::Converter::String8To16(deviceName));
				}

				Face::Status Face::SetDeviceByFormat(VideoFormat *min, VideoFormat *max)
				{
				...
				Face::Status Face::Activate(String8 license)
				{
					if (sm_activated)
						return FSDKE_OK;
				...
				Face::Status Face::Initialize(String8 dataFiles)
				{
					if (sm_initialized)
						return FSDKE_OK;
					Status status;
				...
				Face::Status Face::InitializeCapturing()
				{
				...
				Face::Status Face::FinalizeCapturing()
				{
					if (!sm_capturingInitialized)
						return FSDKE_OK;
